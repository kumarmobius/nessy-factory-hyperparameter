...
'''
    ...
    def run(
        ...
        # 4) instantiate the model with HF-compatible keys
        class RMSNorm(nn.Module):
            def __init__(self, emb_dim, eps=1e-6, bias=False):
                super().__init__()
                self.eps = eps
                self.weight = nn.Parameter(torch.ones(emb_dim))  # HF uses 'weight'
        ...
        class GroupedQueryAttention(nn.Module):
            def __init__(self, d_in, num_heads, num_kv_groups, head_dim=None, qk_norm=False, query_pre_attn_scalar=None, dtype=None):
                ...
                self.q_proj = nn.Linear(d_in, num_heads * self.head_dim, bias=False, dtype=model_dtype)
                self.k_proj = nn.Linear(d_in, num_kv_groups * self.head_dim, bias=False, dtype=model_dtype)
                self.v_proj = nn.Linear(d_in, num_kv_groups * self.head_dim, bias=False, dtype=model_dtype)
                self.o_proj = nn.Linear(num_heads * self.head_dim, d_in, bias=False, dtype=model_dtype)
        ...
        class MLP(nn.Module):
            def __init__(self, cfg_local):
                ...
                self.gate_proj = nn.Linear(emb_dim, hidden_dim, bias=False, dtype=model_dtype)
                self.up_proj = nn.Linear(emb_dim, hidden_dim, bias=False, dtype=model_dtype)
                self.down_proj = nn.Linear(hidden_dim, emb_dim, bias=False, dtype=model_dtype)
        ...
        class TransformerBlock(nn.Module):
            def __init__(self, cfg_local, layer_type):
                ...
                self.self_attn = GroupedQueryAttention(
                    cfg_local["emb_dim"], cfg_local["n_heads"], cfg_local.get("n_kv_groups", 1),
                    cfg_local["head_dim"], cfg_local.get("qk_norm", False),
                    cfg_local.get("query_pre_attn_scalar", None), model_dtype
                )
                # HF-compatible norm names
                self.input_layernorm = RMSNorm(cfg_local["emb_dim"], eps=cfg_local.get("rms_norm_eps", 1e-6))
                self.post_attention_layernorm = RMSNorm(cfg_local["emb_dim"], eps=cfg_local.get("rms_norm_eps", 1e-6))
                self.pre_feedforward_layernorm = RMSNorm(cfg_local["emb_dim"], eps=cfg_local.get("rms_norm_eps", 1e-6))
                self.post_feedforward_layernorm = RMSNorm(cfg_local["emb_dim"], eps=cfg_local.get("rms_norm_eps", 1e-6))
                self.mlp = MLP(cfg_local)
            
            def forward(self, x, cos, sin, mask=None):
                residual = x
                x = self.input_layernorm(x)
                attn_out = self.self_attn(x, cos, sin, mask)
                x = self.post_attention_layernorm(attn_out + residual)
                residual = x
                x = self.pre_feedforward_layernorm(x)
                x = self.post_feedforward_layernorm(self.mlp(x) + residual)
                return x
        ...
        class Gemma3Model(nn.Module):
            def __init__(self, cfg_local):
                ...
                # Create 'model' namespace for HF compatibility
                self.model = nn.Module()
                self.model.embed_tokens = nn.Embedding(vocab_size, emb_dim, dtype=model_dtype)
                self.model.layers = nn.ModuleList([
                    TransformerBlock(cfg_local, layer_type=layer) 
                    for layer in cfg_local["layer_types"]
                ])
                self.model.norm = RMSNorm(emb_dim, eps=cfg_local.get("rms_norm_eps", 1e-6))
                
                self.lm_head = nn.Linear(emb_dim, vocab_size, bias=False, dtype=model_dtype)
                
                self.cos_global, self.sin_global = compute_rope_params(
                    cfg_local["head_dim"], cfg_local["rope_base"], 
                    cfg_local["context_length"], torch.float32
                )
                self.cos_local, self.sin_local = compute_rope_params(
                    cfg_local["head_dim"], cfg_local["rope_local_base"], 
                    cfg_local["sliding_window"], torch.float32
                )
            ...
            def forward(self, input_ids: torch.LongTensor, labels: torch.LongTensor = None):
                x = self.model.embed_tokens(input_ids).to(model_dtype)
                B, T, _ = x.size()
                device = x.device
        ...
        # 5) save artifacts with HF-compatible keys
        os.makedirs(os.path.dirname(model_weights_out) or ".", exist_ok=True)
        torch.save(model.state_dict(), model_weights_out)
        
        print(f"✓ Saved checkpoint with HF-compatible keys to: {model_weights_out}")
        print("\nKey structure:")
        for key in sorted(model.state_dict().keys())[:10]:
            print(f"  {key}")
        if len(model.state_dict().keys()) > 10:
            print(f"  ... and {len(model.state_dict().keys()) - 10} more keys")

        cfg_json = {**cfg, "dtype": ("float16" if cfg["dtype"] == torch.float16 else "float32")}
